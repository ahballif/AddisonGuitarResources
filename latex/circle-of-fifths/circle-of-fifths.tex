...
There is a lot of structure in music. As this structure is commonly reused in different pieces, it is very useful to be aware of many of the patterns that go on. In this document I hope to explain the concept of keys, how notes fit into each key, and what the purpose of the chords is for a given key. This document should teach you how to know if a certain chord should be major or minor in a given context and should hopefully help open your ears to hear music in a new way. 

\section{Background}
Before we can talk about keys we need to first talk about notes. 
A note is a pitch with a specific frequency. On the other hand, chords are sounds that play a collection of frequencies as the same time. 
When we sing, we sing notes. We cannot sing chords, but instruments such as the paino or the guitar can play chords. 

Each note has a name. Understanding the names of the notes is an essential starting place. The chords are named based on the notes they contain, so we must first know the names of the notes. 
Below is a piano keyboard that gives the ordering of the different notes. 
...
Because these notes can be labeled above or below, they have two names to represent the same thing. This may seem redundant, but it is important to have both when we get to creating scales of notes. 
...
This choice creates some very useful structure. Every time we move clockwise by one step, the major scale key signature gains one sharp (or loses one flat).
If we move counter clockwise, the major scale gains a flat (or loses a sharp). 
In the previous section we mentioned that knowing how many sharps or flats a major key has determines the key.
The order that we add sharps and flats is also determined by the circle of fifths. Starting with F$\sharp$, we add sharps in the order F, C, G, $\dots$. When adding with flats we start with B$\flat$ and move the other direction. 
...
Notice the structure. The four chords are determined by the root and its two neighbors, as well as the minor chord 4 notes in the clockwise direction. Thus in G major we would have G, C, D, and E minor as our four chords. If we keep the structure the same but rotate only the notes, we can find the 4 chords for any possible key. 
